...
class Play
  ...
  def self.find_by_title(title)
    title_info = PlayDBConnection.instance.execute(<<-SQL, title)
      SELECT 
        * 
      FROM 
        plays 
      WHERE 
        title = ?
      SQL
    return nil unless title_info.length > 0
    Play.new(title_info.first)
  end
  def self.find_by_playwright(name)
      playwright = Playwright.find_by_name(name)
      raise "Playwright not in DB" unless playwright
